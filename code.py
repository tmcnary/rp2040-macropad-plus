...
- For macros in the main folder, pressing the encoder opens a menu to choose macros or subfolders.
- For macros in subfolders, turning the encoder directly switches between macros in that folder.
- Pressing the encoder always opens the main menu to choose from all macros and subfolders.
"""
...
def read_macro_files(folder=MACRO_FOLDER):
    """ Read all the macro key setups from .py files in a given folder """
    apps = []
...
# Load all the macro key setups from .py files in MACRO_FOLDER and subfolders
